/////////////////////////////////////////////////////////
//程序名称：E3_1编程实现直线段的裁剪算法（鼠标）（用Choen-Sutherlan法和Liang-Barsky直线裁剪方法）
...
for(int v = 0; v <maxn; v++) if(G[u][v] && !vis[u][v]) 
{
     ...
     //BresenhamLine((int)p[u].x,(int)p[u].y,(int)p[v].x,(int)p[v].y,RED);// BresenhamLine画线方法
       //getch(); //加上这一句变成 手动
	//C_S_LineClip((int)p[u].x,(int)p[u].y,(int)p[v].x,(int)p[v].y);//Choen-Sutherlan法（本质上是进行边绘制边裁剪）
	...
	while(true)
	{
		...
		switch(m.uMsg)
		{
			...
			case WM_LBUTTONDOWN:
				...
				if(n%2==1){
				XL=m.x;YB=m.y;//手动选取裁剪方式，但只能选择一次，像素不能够进行覆盖
				}
				else	if(n%2==0){XR=m.x;YT=m.y;
				if(XL>XR){d=XL;XL=XR;XR=d;}
				if(YB>YT){d=YB;YB=YT;YT=d;}
				euler(0);//欧拉回路的构建
				}
				break;
			case WM_RBUTTONUP:
